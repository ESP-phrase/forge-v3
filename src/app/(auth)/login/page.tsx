...
import { isGitHubAuthConfigured, isGoogleAuthConfigured } from "@/lib/auth";
import { SubmitButton } from "./SubmitButton";

export default async function LoginPage({
  searchParams,
}: {
  searchParams: Promise<{ error?: string; magic?: string; email?: string; next?: string; showPassword?: string }>;
}) {
  ...
  return (
    <>
      <div className="text-center mb-7">
        ...
        <p className="text-muted text-sm mt-2">Enter your email — we'll send you a link.</p>
      </div>
        ...
        <div className="relative bg-card-grad rounded-2xl p-7 shadow-panel">
          ...
          {justSent ? (
            <div className="bg-accent-dim text-accent border border-accent-border rounded-lg px-3.5 py-2.5 mb-4 text-sm">
              Link sent — check {magicEmail || "your inbox"}. No password needed.
            </div>
          ) : null}

          {/* Magic link — the only visible option by default */}
          <form action={sendMagicLinkAction}>
            ...
            <SubmitButton idleLabel="Continue" busyLabel="Sending link…" />
          </form>

          {/* OAuth — only when configured */}
          {showOAuth ? (
            <>
              ...
              {showGoogle ? (
                <form action={signInWithGoogleAction}>
                  <button type="submit" className="relative w-full flex items-center justify-center gap-3 px-4 py-4 bg-white text-gray-900 rounded-xl font-bold text-base hover:bg-gray-100 transition-colors">
              ...
              {showGitHub ? (
                <form action={signInWithGitHubAction} className={showGoogle ? "mt-2" : ""}>
          ...
          {/* Password — hidden behind a toggle */}
          {showPassword ? (
            <>
              <div className="flex items-center gap-3 my-5">
                <div className="flex-1 h-px bg-border" />
                <span className="text-muted-2 text-[0.6rem] uppercase tracking-[0.18em] font-bold">or password</span>
                <div className="flex-1 h-px bg-border" />
              </div>
              <form action={signInWithPasswordAction} className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <input name="password" type="password" required minLength={8} autoComplete="current-password" placeholder="Password" className="w-full pl-11 pr-3 py-3 bg-bg border border-border rounded-xl text-sm text-text focus:outline-none focus:border-accent-border placeholder:text-muted-2" />
                </div>
          ...
          <div className="text-center mt-4 text-muted text-xs">
            <Link
              href={`/login${showPassword ? "" : "?showPassword=1"}${next ? (showPassword ? "?" : "&") + "next=" + encodeURIComponent(next) : ""}`}
              className="text-accent font-semibold hover:underline no-underline"
            >
              {showPassword ? "← Back to email" : "Use password instead"}
            </Link>
          </div>
