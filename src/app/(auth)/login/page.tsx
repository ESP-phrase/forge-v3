    ...
    <>
      ...
      <div className="relative">
        ...
        <div className="relative bg-card-grad rounded-2xl p-6 shadow-panel">
          ...
          {justSent ? (
            <div className="bg-accent-dim text-accent border border-accent-border rounded-lg px-4 py-3 mb-4 text-sm text-center">
              <div className="font-bold mb-1">Check your inbox</div>
              We sent a sign-in link to <strong>{magicEmail || "your email"}</strong>.<br />
              Click the link and you're in. No password. Ever.
            </div>
          ) : null}
          ...
          {hasOAuth ? (
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-muted-2 text-[0.6rem] uppercase tracking-[0.18em] font-bold">or use email</span>
              <div className="flex-1 h-px bg-border" />
            </div>
          ) : null}

          {/* Step 2: Email — no password */}
          ...
          <div className="text-center mt-4 pt-3 border-t border-border">
            <Link
              ...
              className="text-muted text-xs hover:text-text transition-colors no-underline"
            >
              {showPassword ? "← Back" : "Sign in with password instead"}
            </Link>
          </div>

          {/* Hidden password form */}
          {showPassword ? (
            <form action={signInWithPasswordAction} className="space-y-3 mt-3">
              {next ? <input type="hidden" name="next" value={next} /> : null}
              <div className="relative">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted" aria-hidden>
                  <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
              ...
              <div className="relative">
                ...
                <input name="password" type="password" required minLength={8} autoComplete="current-password" placeholder="Password" className="w-full pl-10 pr-3 py-2.5 bg-bg border border-border rounded-xl text-sm text-text focus:outline-none focus:border-accent-border placeholder:text-muted-2" />
              </div>
      ...
      <div className="flex items-center justify-center gap-5 text-muted text-xs mt-6">
