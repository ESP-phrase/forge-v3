...
function convertAnthropicToOpenRouter(modelId: string): string {
  // Strip trailing date suffix like -YYYYMMDD
  const stripped = modelId.replace(/-\d{8}$/, "");
  // Convert the final "X-Y" version pair to "X.Y"
...
export function resolveModel(modelId: string): string {
  ...
  return ANTHROPIC_TO_OPENROUTER[modelId] ?? convertAnthropicToOpenRouter(modelId);
}
