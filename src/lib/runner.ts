...
export async function runOneForSite(
  ...
  // Enhanced quality gates — catch AI slacking on visual formatting
  const hasTldr = /class="tldr"/i.test(article.html);
  const calloutCount = (article.html.match(/class="callout/i) ?? []).length;
  const pullQuoteCount = (article.html.match(/class="pull-quote"/i) ?? []).length;
  const hasVisual = /class="comparison"|class="stat-row"|class="steps"/i.test(article.html);
  const markCount = (article.html.match(/<mark>/gi) ?? []).length;
  const h2Count = (article.html.match(/<h2[^>]*>/gi) ?? []).length;

  if (!hasTldr) {
    qualityWarning = (qualityWarning ? `${qualityWarning}; ` : "") + "missing TL;DR block";
  }
  if (calloutCount < 2) {
    qualityWarning = (qualityWarning ? `${qualityWarning}; ` : "") + `only ${calloutCount} callout(s) (need 2+)`;
  }
  if (pullQuoteCount < 1) {
    qualityWarning = (qualityWarning ? `${qualityWarning}; ` : "") + "missing pull-quote";
  }
  if (!hasVisual) {
    qualityWarning = (qualityWarning ? `${qualityWarning}; ` : "") + "no visual element (table, stat row, or steps)";
  }
  if (markCount < 5) {
    qualityWarning = (qualityWarning ? `${qualityWarning}; ` : "") + `only ${markCount} <mark> tags (need 8+)`;
  }
  if (h2Count < 2) {
    qualityWarning = (qualityWarning ? `${qualityWarning}; ` : "") + `only ${h2Count} H2 section(s) (need 3+)`;
  }
